...
def main():
    for label, p in [
        ("7d Sep 7–13", ROOT / "rolling-7d/Ads - campaigns_api_2026-09-07_to_2026-09-13.csv"),
        ("28d Aug 17–Sep 13", ROOT / "rolling-28d/Ads - campaigns_api_2026-08-17_to_2026-09-13.csv"),
        ("30d Aug 15–Sep 13", ROOT / "rolling-30d/Ads - campaigns_api_2026-08-15_to_2026-09-13.csv"),
        ("Sep MTD", ROOT / "sep-mtd/Ads - campaigns_api_2026-09-01_to_2026-09-13.csv"),
    ]:
        rolled, tot = roll_campaigns(p)
        print_table(label, rolled, tot)

    conv_actions(ROOT / "rolling-7d/Ads - conversion-actions_api_2026-09-07_to_2026-09-13.csv", "7d")
    conv_actions(ROOT / "rolling-28d/Ads - conversion-actions_api_2026-08-17_to_2026-09-13.csv", "28d")
    conv_actions(ROOT / "rolling-30d/Ads - conversion-actions_api_2026-08-15_to_2026-09-13.csv", "30d")
    conv_actions(ROOT / "sep-mtd/Ads - conversion-actions_api_2026-09-01_to_2026-09-13.csv", "Sep MTD")

    print("\n=== DEVICES ===")
    for label, p in [
        ("7d", ROOT / "rolling-7d/Ads - devices_api_2026-09-07_to_2026-09-13.csv"),
        ("30d", ROOT / "rolling-30d/Ads - devices_api_2026-08-15_to_2026-09-13.csv"),
    ]:
    ...
    print("\n=== 01 KEYWORD CONV (Jul 9–Sep 13) ===")
    kwp = ROOT / "Ads - keywords_api_2026-07-09_to_2026-09-13.csv"
    ...
    print("\n=== SEARCH TERMS converters (Jul 9–Sep 13) ===")
    stp = ROOT / "Ads - search-terms_api_2026-07-09_to_2026-09-13.csv"
    ...
    with stp.open() as fh:
        for row in csv.DictReader(fh):
            ...
            if row["segments.date"] >= "2026-09-07":
                e = st7[term]
    ...
    with (ROOT / "Ads - campaign-shared-sets_api_snapshot_2026-09-13.csv").open() as fh:
        for row in csv.DictReader(fh):
            camp_attach[row["campaign.name"]].append(row["shared_set.name"])

    ag_negs = defaultdict(list)
    with (ROOT / "Ads - negative-keywords_api_snapshot_2026-09-13.csv").open() as fh:
        for row in csv.DictReader(fh):
    ...
    enabled = []
    skipped_ag = 0
    with (ROOT / "Ads - keyword-settings_api_snapshot_2026-09-13.csv").open() as fh:
        for row in csv.DictReader(fh):
