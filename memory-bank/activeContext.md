...
- [x] Foreign-into-NY state landing pages - five state-specific pages at `/foreign-pllc/{state}` (New Jersey, Pennsylvania, Florida, Texas, Connecticut) with unique copy, exact PLLC/PC pricing, FAQs, SEO metadata, structured data, and sitemap entries
...
- [x] Foreign state page UX refinements - removed duplicate document cards; "What we need from you" lists customer-provided info only (we obtain standing/certified formation docs); cross-sell link to `/virtual-address-services` for NY business address and registered agent
...
- **Foreign-into-NY page pattern**: State pages live at `/foreign-pllc/{slug}` with per-state content objects and shared `StateForeignQualificationPage` layout. Hero uses `ForeignStateSelector` (not breadcrumb). Pricing split by foreign PLLC vs foreign PC path. Copy rules: unique state-aware content, no em dashes, do not promise publication included unless product confirms it. Customer checklist titled **What we need from you** — only info the client supplies (entity name, state ID if available, entity type, license info, assumed name preference). We obtain home-state standing and certified formation documents; that belongs in **What we check before filing**, not the customer list. NY address/registered agent bullet is shared in the layout component and links to `/virtual-address-services`. Key files: `web/src/lib/constants/foreign-states.ts`, `web/src/app/foreign-pllc/_components/foreign-state-selector.tsx`, `web/src/app/foreign-pllc/_components/state-foreign-qualification-page.tsx`.
